...
def compute_scalp_signal(
    ...
    *,
    ...
    if len(ohlcv) < 60:
        return SignalResult(symbol, "NEUTRAL", "NONE", 0, "Not enough data", None, None, None, None, None, None, "OFF", {})
    ...
    extras: Dict[str, Any] = {
        ...
        "vwap_approach_long": False,
        "vwap_approach_short": False,
    }
    ...
    # Pre-emptive VWAP proximity triggers (used for PRE alerts to avoid being late)
    vwap_prox_long = False
    vwap_prox_short = False
    try:
        vwap_last = float(vwap_use.iloc[-1])
        close_last = float(close.iloc[-1])
        if atr_last and np.isfinite(atr_last) and atr_last > 0:
            prox = 0.20 * float(atr_last)
            # Approaching VWAP from below (bull) / from above (bear)
            vwap_prox_long = bool(was_below_vwap and (not reclaim_vwap) and (close_last < vwap_last) and ((vwap_last - close_last) <= prox) and ((rsi_fast.diff().tail(3).mean() > 0) or (macd_hist.diff().tail(3).mean() > 0)))
            vwap_prox_short = bool(was_above_vwap and (not reject_vwap) and (close_last > vwap_last) and ((close_last - vwap_last) <= prox) and ((rsi_fast.diff().tail(3).mean() < 0) or (macd_hist.diff().tail(3).mean() < 0)))
    except Exception:
        vwap_prox_long = False
        vwap_prox_short = False
    extras["vwap_prox_long"] = bool(vwap_prox_long)
    extras["vwap_prox_short"] = bool(vwap_prox_short)

    # Pre-trigger helpers (more pre-emptive, still quantifiable):
    # If price is *approaching* VWAP and momentum is improving, we can emit a PRE alert
    # so the trader can stage orders before the reclaim/reject prints.
    vwap_approach_long = False
    vwap_approach_short = False
    try:
        if atr_last and np.isfinite(vwap_use.iloc[-1]):
            dist = float(vwap_use.iloc[-1] - close.iloc[-1])
            # approaching from below (long) within 0.20 ATR with improving RSI/MACD slope
            if was_below_vwap and (close.iloc[-1] < vwap_use.iloc[-1]) and (dist >= 0):
                rsi_slope = float(rsi_fast.diff().tail(3).mean())
                macd_slope = float(macd_hist.diff().tail(3).mean())
                if dist <= 0.20 * float(atr_last) and (rsi_slope > 0 or macd_slope > 0):
                    vwap_approach_long = True
            # approaching from above (short) within 0.20 ATR with worsening RSI/MACD slope
            dist2 = float(close.iloc[-1] - vwap_use.iloc[-1])
            if was_above_vwap and (close.iloc[-1] > vwap_use.iloc[-1]) and (dist2 >= 0):
                rsi_slope = float(rsi_fast.diff().tail(3).mean())
                macd_slope = float(macd_hist.diff().tail(3).mean())
                if dist2 <= 0.20 * float(atr_last) and (rsi_slope < 0 or macd_slope < 0):
                    vwap_approach_short = True
    except Exception:
        vwap_approach_long = vwap_approach_short = False

    # Persist approach flags (used in UI + PRE alert gating)
    extras["vwap_approach_long"] = bool(vwap_approach_long)
    extras["vwap_approach_short"] = bool(vwap_approach_short)
    ...
    elif vwap_approach_long and not reclaim_vwap:
        _add("LONG", "vwap_approach", 12, f"Approaching VWAP reclaim ({vwap_logic})")
    ...
    elif vwap_approach_short and not reject_vwap:
        _add("SHORT", "vwap_approach", 12, f"Approaching VWAP rejection ({vwap_logic})")
    ...
    # PRE/CONFIRMED alert tiers
    # - CONFIRMED: all requirements satisfied (volume rule, confirmations, hard triggers)
    # - PRE: score is strong and we have at least one quantifiable trigger, but we're still waiting on
    #        confirmations (e.g., volume, VWAP reclaim print, ORB break, divergence follow-through, etc.)
    blockers: List[str] = []
    confirmed_ok = True
    ...
    if int(cfg.get("require_volume", 0)) == 1 and (not volume_event) and (not strong_pro_confluence):
        confirmed_ok = False
        blockers.append("Missing volume confirmation")

    # Primary trigger must exist to anchor an entry plan.
    # For PRE alerts we allow VWAP-approach as a trigger too.
    primary_trigger = bool(vwap_event or rsi_event or macd_event or pro_trigger)
    pre_primary_trigger = bool(primary_trigger or vwap_approach_long or vwap_approach_short)
    extras["primary_trigger"] = bool(primary_trigger)
    extras["pre_primary_trigger"] = bool(pre_primary_trigger)

    if not primary_trigger:
        confirmed_ok = False
        blockers.append("No primary trigger (VWAP/RSI/MACD/Pro)")

    # Require enough confirmations for CONFIRMED.
    if confirmation_score < confirm_threshold:
        confirmed_ok = False
        blockers.append(f"Confirmations {confirmation_score}/{confirm_threshold}")

    # Classic hard requirements (RTH only) unless Pro trigger exists.
    ...
    if hard_vwap and (not vwap_event) and (not pro_trigger):
        confirmed_ok = False
        blockers.append("No VWAP reclaim/rejection event")
    if hard_rsi and (not rsi_event) and (not pro_trigger):
        confirmed_ok = False
        blockers.append("No RSI snap/downshift event")
    if hard_macd and (not macd_event) and (not pro_trigger):
        confirmed_ok = False
        blockers.append("No MACD histogram turn event")
    ...
    extras["confirmed_ok"] = bool(confirmed_ok)
...
# ATR-normalized score calibration (per ticker)
    ...
    if long_score < min_score and short_score < min_score:
        ...
        return SignalResult(symbol, "NEUTRAL", "NONE", _cap_score(max(long_score, short_score)), reason, None, None, None, None, last_price, last_ts, session, extras)

    bias = "LONG" if long_score >= short_score else "SHORT"
    setup_score = _cap_score(max(long_score, short_score))

    # Determine alert tier (PRE vs CONFIRMED).
    pre_primary_trigger = bool(extras.get("pre_primary_trigger", False))
    if bool(extras.get("confirmed_ok", False)):
        tier = "CONFIRMED"
    elif pre_primary_trigger:
        tier = "PRE"
    else:
        tier = "NONE"
    ...
    if tier == "PRE":
        # Make it explicit in UI + email that this is an early heads-up.
        reason = "PRE-ALERT: " + ("; ".join(extras.get("blockers", [])) + (" | " if extras.get("blockers") else "")) + reason
    elif tier == "NONE":
        # High raw points but no trigger — treat as non-actionable.
        extras["decision"] = {"long": long_score, "short": short_score, "min": min_score}
        return SignalResult(symbol, "NEUTRAL", "NONE", setup_score, "No actionable trigger", None, None, None, None, last_price, last_ts, session, extras)
    ...
    return SignalResult(symbol, bias, tier, setup_score, reason, float(entry_px), float(stop_px), float(tp1) if tp1 is not None else None, float(tp2) if tp2 is not None else None, last_price, last_ts, session, extras)
